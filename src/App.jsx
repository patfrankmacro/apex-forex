          ...
          <div style={{display:"flex",flexDirection:"column",gap:6}}>
            ...
            <div style={{background:"#0c0c18",borderRadius:6,padding:"8px 10px",borderLeft:"3px solid "+(t.cotBias==="HAUSSIER"?"#4ade80":"#f87171")}}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div style={{fontSize:9,color:"#94a3b8"}}>
                <div style={{fontSize:9,color:"#94a3b8"}}>
                  <FlagImg code={t.base} size={12} /> <span style={{color:t.bCotRaw?.signal?.includes("HAUSSIER")?"#4ade80":"#f87171"}}>{(t.bCotRaw?.signal||"—").replace("_"," ")}</span> <span style={{color:"#4a5070"}}>({t.bCotRaw?.chgNet>=0?"+":""}{(t.bCotRaw?.chgNet||0).toLocaleString()})</span>
                  &nbsp;vs&nbsp;
                  <FlagImg code={t.quote} size={12} /> <span style={{color:t.qCotRaw?.signal?.includes("HAUSSIER")?"#4ade80":"#f87171"}}>{(t.qCotRaw?.signal||"—").replace("_"," ")}</span> <span style={{color:"#4a5070"}}>({t.qCotRaw?.chgNet>=0?"+":""}{(t.qCotRaw?.chgNet||0).toLocaleString()})</span>
                </div>
                <div style={{fontSize:10,fontWeight:700,color:t.cotBias==="HAUSSIER"?"#4ade80":"#f87171"}}>
                  {t.cotBias} — {t.cotStrength}
                </div>
