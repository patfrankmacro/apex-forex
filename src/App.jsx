...
function SwingMidView() {
  return (
    <div>
      ...
      <div style={{fontSize:9, color:"#c084fcaa", fontWeight:700, letterSpacing:2, marginBottom:14}}>{"LA TENDANCE DE FOND · M30/H1 · CONVERGENCE TOTALE"}</div>
      {/* SWING MID */}
  ...
  const analyze = () => {
    try {
      ...
      try {
        ...
        else degrade.push("COT absent — le bonus Fonds ne peut pas être évalué");
      } catch(e){}
      ...
      if (top.length===0){ setResult({error:"AUCUNE opportunité APEX pour ta session — aucune de tes 7 paires n'a le SIGNAL (① divergence ≥4r + ② Top 5 Gainers/Losers). Pas de trade = bonne décision.", strongest, weakest, usdRank: (strength.indexOf("USD")>=0?strength.indexOf("USD")+1:null), diag7: diagnostic, degrade}); return; }
    ...
    <div style={{padding:"12px 14px", background:"#0a1628", borderRadius:8, border:"1px solid #fbbf2455", marginBottom:14}}>
          ...
          <div style={{marginTop:10, padding:"10px", background: has?"#052010":"#0f1622", borderRadius:6, border:"1px solid "+(has?"#4ade8055":"#1e3a5f")}}>
            ...
            {has ? h.signaux.map((s,i)=>(
              <div key={i} style={{padding:"6px 8px", background:"#0a2818", borderRadius:4, marginBottom:4, fontSize:9, color:"#86efac", fontWeight:700}}>
                🎯 {s.pair.slice(0,3)}/{s.pair.slice(3,6)} {s.dir==="LONG"?"▲ ACHAT":"▼ VENTE"} · divergence {s.gap}r · SIGNAL — surveille le drapeau, entre à la cassure (fin NY/Tokyo)
              </div>
        ...
        <div style={{marginTop:10, padding:"10px", background:"#0a1020", borderRadius:6, border:"1px solid #1e3a5f"}}>
          ...
          {result.usdRank!=null && (
            <div style={{marginTop:8, padding:"7px 9px", background: result.usdRank<=2?"#200505":result.usdRank>=7?"#1a1500":"#0f1622", borderRadius:4, borderLeft:"3px solid "+(result.usdRank<=2?"#f87171":result.usdRank>=7?"#fbbf24":"#334155")}}>
              ...
              {(result.usdRank<=2||result.usdRank>=7) && <div style={{fontSize:7.5, color:TEXT_DIM, marginTop:3, lineHeight:1.5}}>{"Pas de Top 5 MarketMilk pour l'or : son ② c'est TON H1 (pôle continu de 3h à maintenant). Même séquence : drapeau, cassure le soir, stop sur le drapeau, 1-3 jours. Le COMEX donne du volume à l'or toute la session NY."}</div>}
            </div>
          )}
        </div>
      )}
      ...
      {result && !result.error && (
        <div style={{marginTop:10}}>
          ...
          <div style={{fontSize:9, color:"#fbbf24", fontWeight:700, marginBottom:8}}>🎯 {result.top.length} OPPORTUNITÉ{result.top.length>1?"S":""} APEX (SIGNAL ① + ②)</div>
          {result.top.map((o,i)=>{
    ...
    <div style={{padding:16, maxWidth:760, margin:"0 auto"}}>
      ...
      <div style={{padding:"12px 14px", background:"#0a1020", borderRadius:8, border:"1px solid #f59e0b55", marginBottom:14}}>
        ...
        <div style={{display:"flex", flexDirection:"column", gap:6, marginBottom:10}}>
          ...
          <div style={{display:"flex", gap:8, padding:"6px 9px", background:"#0a1a2e", borderRadius:5}}><span style={{color:APX.obs, fontWeight:700, fontSize:9, minWidth:62}}>6h-8h</span><span style={{fontSize:8.5, color:TEXT, lineHeight:1.5}}>TA PRÉPARATION : ce qui a DÉJÀ eu lieu — news à fort impact de la nuit (Asie) et du matin (Londres) + commentaires des banques centrales (session wraps). Le POURQUOI derrière ce que les desks font depuis 3h.</span></div>
      ...
      <div style={{padding:"12px 14px", background:"#0a1628", borderRadius:8, border:"1px solid #fbbf2444", marginBottom:14}}>
        ...
        <div style={{display:"flex", flexDirection:"column", gap:10}}>
          ...
          <div style={{padding:"8px 10px", background:"#001018", borderRadius:6, borderLeft:"3px solid #f87171"}}>
            ...
            C'est exactement ce que ton bonus retail ≥70% capte : quand le retail est massivement d'un côté, les deux mécaniques sont en place. Le mouvement institutionnel peut partir.</div>
          </div>
    ...
    <div style={{ minHeight:"100vh", background:BG, fontFamily:"'IBM Plex Mono',monospace", color:TEXT, fontSize:12 }}>
      ...
      <div style={{ background:BG2, borderBottom:`1px solid ${BORDER}`, padding:"10px 16px", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:8 }}>
        ...
        <div style={{ display:"flex", gap:6, flexWrap:"wrap", alignItems:"center" }}>
          ...
          <button onClick={()=>setView("swingmid")} style={{ padding:"6px 12px", borderRadius:6, fontSize:10, fontWeight:800, letterSpacing:1, cursor:"pointer", border: view==="swingmid"?"2px solid #c084fc":"2px solid #c084fc66", background: view==="swingmid"?"#c084fc":"#14081f", color: view==="swingmid"?"#14081f":"#c084fc", fontFamily:"'IBM Plex Mono'", boxShadow:"0 0 8px rgba(192,132,252,0.3)" }}>{"🌙 SWING MID"}</button>
