...
function rankStocks(rows, tech) {
  ...
  return rows.map(r=>{
    ...
    // 3 MOIS = ancrage de solidite (tendance solide vs soubresaut)
    if(r.quart!=null){ if(r.quart>100){mom+=22;why.push("tendance très solide (3M +"+r.quart+"%)");} else if(r.quart>50){mom+=16;why.push("tendance solide (3M +"+r.quart+"%)");} else if(r.quart>20)mom+=9; else mom+=3; }
    // 5 JOURS = acceleration recente
...
function StockAnalyseView() {
  ...
  return (
    <div style={{maxWidth:540, margin:"0 auto", padding:"0 4px"}}>
      <style>{`@keyframes floatBubble{0%{transform:translateY(0)}50%{transform:translateY(-7px)}100%{transform:translateY(0)}}@keyframes glowB{0%,100%{box-shadow:0 0 8px #c9a84c66}50%{box-shadow:0 0 18px #c9a84ccc}}`}</style>
