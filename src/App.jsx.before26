...
function getRegime(data, code) {
  ...
  const inflationHigh = coreNow !== null && coreNow > target + 0.3;
  const growUp = !svcContraction && (growScore !== null && growScore > 0.05);
  const growDown = svcContraction || (growScore !== null && growScore < -0.05);
  const infUp = inflationHigh && (infScore !== null && infScore > 0.0);
  const infDown = !inflationHigh || (infScore !== null && infScore < 0.0);
  if (growUp && infUp)   return REGIMES.SURCHAUFFE;
  if (growUp && infDown) return REGIMES.GOLDILOCKS;
  if (growDown && inflationHigh) return REGIMES.STAGFLATION;
  if (growDown && infDown) return REGIMES.RECESSION;
}
              ...
              return (
                <div key={id} style={{ display:"flex", gap:8, marginBottom:6, padding:"6px 8px", background:BG2, borderRadius:2, borderLeft:`2px solid ${col}55` }}>
                  <div style={{ flex:1 }}>
                    ...
                    {exp!==null&&<span style={{ color:TEXT_DIM, fontSize:9 }}> (exp:{exp}) </span>}
                    {s!==null&&s!==0&&<span style={{ color:col, fontSize:9, fontWeight:700 }}>{beat?"BEAT ✓":"MISS ✗"} ({s>0?"+":""}{s.toFixed(2)})</span>}
                    {s===0&&<span style={{ color:"#888899", fontSize:9 }}> NEUTRE</span>}
                  </div>
                </div>
              );
            })}
            ...
            {["unemp","svc","mfg","rate"].map(id => {
              ...
              return (
                <div key={id} style={{ display:"flex", gap:8, marginBottom:6, padding:"6px 8px", background:BG2, borderRadius:2, borderLeft:`2px solid ${col}55` }}>
                  <div style={{ flex:1 }}>
                    <span style={{ color:TEXT, fontWeight:600, fontSize:10 }}>{ind.label}: {now}{ind.unit}</span>
                    {id==="rate"&&prior!==null&&<span style={{ color:TEXT_DIM, fontSize:9 }}> (préc:{prior}%) </span>}
                    {id!=="rate"&&exp!==null&&<span style={{ color:TEXT_DIM, fontSize:9 }}> (exp:{exp}) </span>}
                    {s!==null&&s!==0&&<span style={{ color:col, fontSize:9, fontWeight:700 }}>{beat?"BEAT ✓":"MISS ✗"} ({s>0?"+":""}{s.toFixed(2)})</span>}
                    {s===0&&<span style={{ color:"#888899", fontSize:9 }}> NEUTRE</span>}
                  </div>
                </div>
