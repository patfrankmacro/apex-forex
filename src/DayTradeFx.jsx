  ...
  return (
    <div style={{maxWidth:520, margin:"0 auto"}}>
      ...
      <div style={{padding:"12px 14px", background:"#0a1220", borderRadius:8, border:"1px solid #1e3a5f", marginBottom:12}}>
        <div style={{fontSize:11, color:"#7dd3fc", fontWeight:700, marginBottom:4}}>{"\ud83d\uddd3\ufe0f L'AGENDA DES DESKS — LEUR JOURNÉE, TON RÔLE"}</div>
        <div style={{fontSize:8, color:TEXT_DIM, lineHeight:1.5, marginBottom:8}}>{"Leur horaire est le MÊME chaque jour. Tu te cales dessus : tu observes quand ils travaillent, tu entres quand ils te laissent monter (8h ou 9h30)."}</div>
        <div style={{marginBottom:7, paddingLeft:8, borderLeft:"2px solid #64748b"}}><div style={{fontSize:8.5, color:"#64748b", fontWeight:700}}>{"3h \u00b7 Ils piègent"}</div><div style={{fontSize:8, color:TEXT, lineHeight:1.5}}>{"Ouverture Londres, fausse cassure pour chasser les stops. Le premier mouvement MENT."}</div><div style={{fontSize:8, color:TEXT_DIM, lineHeight:1.5}}>{"\ud83e\udde0 Toi : tu dors. Jamais de position ici."}</div></div>
        <div style={{marginBottom:7, paddingLeft:8, borderLeft:"2px solid #38bdf8"}}><div style={{fontSize:8.5, color:"#38bdf8", fontWeight:700}}>{"4h-7h30 \u00b7 Ils chargent (le PÔLE)"}</div><div style={{fontSize:8, color:TEXT, lineHeight:1.5}}>{"Tranches dans la vraie direction = le pôle se dessine sur ton M15."}</div><div style={{fontSize:8, color:TEXT_DIM, lineHeight:1.5}}>{"\ud83e\udde0 Toi : tu lis le POURQUOI (news, session wraps)."}</div></div>
        <div style={{marginBottom:7, paddingLeft:8, borderLeft:"2px solid #38bdf8"}}><div style={{fontSize:8.5, color:"#38bdf8", fontWeight:700}}>{"7h30-8h \u00b7 Ils soufflent (le flag)"}</div><div style={{fontSize:8, color:TEXT, lineHeight:1.5}}>{"Pause avant NY : le prix drifte, le flag se forme."}</div><div style={{fontSize:8, color:TEXT_DIM, lineHeight:1.5}}>{"\ud83e\udde0 Toi : tu traces la jambe et les bornes du flag."}</div></div>
        <div style={{marginBottom:7, paddingLeft:8, borderLeft:"2px solid #34d399"}}><div style={{fontSize:8.5, color:"#34d399", fontWeight:700}}>{"\ud83d\ude80 8h \u00b7 CASSURE 8h"}</div><div style={{fontSize:8, color:TEXT, lineHeight:1.5}}>{"Le forex NY ouvre, ils poussent leur dernière tranche : le flag CASSE dans le sens du pôle."}</div><div style={{fontSize:8, color:TEXT_DIM, lineHeight:1.5}}>{"\ud83e\udde0 Toi : ENTRÉE possible — clôture M15 hors du flag, stop large."}</div></div>
        <div style={{marginBottom:7, paddingLeft:8, borderLeft:"2px solid #f59e0b"}}><div style={{fontSize:8.5, color:"#f59e0b", fontWeight:700}}>{"8h30 \u00b7 Les news US"}</div><div style={{fontSize:8, color:TEXT, lineHeight:1.5}}>{"CPI, NFP, claims tombent. L'humeur risk-on/off peut basculer."}</div><div style={{fontSize:8, color:TEXT_DIM, lineHeight:1.5}}>{"\ud83e\udde0 Toi : tu vérifies que ton \u2462 tient toujours."}</div></div>
        <div style={{marginBottom:7, paddingLeft:8, borderLeft:"2px solid #fbbf24"}}><div style={{fontSize:8.5, color:"#fbbf24", fontWeight:700}}>{"9h-9h30 \u00b7 Le pôle est mûr"}</div><div style={{fontSize:8, color:TEXT, lineHeight:1.5}}>{"Londres a poussé 6h30, NY a testé 1h30 sans inverser."}</div><div style={{fontSize:8, color:TEXT_DIM, lineHeight:1.5}}>{"\ud83e\udde0 Toi : tu scannes (\u2460 \u22653r + \u2462 + pas Least Volatile)."}</div></div>
        <div style={{marginBottom:7, paddingLeft:8, borderLeft:"2px solid #34d399"}}><div style={{fontSize:8.5, color:"#34d399", fontWeight:700}}>{"\ud83d\ude80 9h30 \u00b7 RETRACEMENT 9h30"}</div><div style={{fontSize:8, color:TEXT, lineHeight:1.5}}>{"Le NYSE ouvre, dernier balayage (ramasse les stops) puis repart dans le sens du pôle."}</div><div style={{fontSize:8, color:TEXT_DIM, lineHeight:1.5}}>{"\ud83e\udde0 Toi : ENTRÉE possible — rebond Fib (38.2/50/61.8%) sur M5, stop sous le retracement."}</div></div>
        <div style={{marginBottom:7, paddingLeft:8, borderLeft:"2px solid #38bdf8"}}><div style={{fontSize:8.5, color:"#38bdf8", fontWeight:700}}>{"9h30-16h \u00b7 Ils portent"}</div><div style={{fontSize:8, color:TEXT, lineHeight:1.5}}>{"Le retracement épuisé, NY porte le mouvement l'après-midi."}</div><div style={{fontSize:8, color:TEXT_DIM, lineHeight:1.5}}>{"\ud83e\udde0 Toi : tu ne touches à rien. Stop et target travaillent."}</div></div>
        <div style={{marginBottom:7, paddingLeft:8, borderLeft:"2px solid #f87171"}}><div style={{fontSize:8.5, color:"#f87171", fontWeight:700}}>{"17h \u00b7 Ils clôturent"}</div><div style={{fontSize:8, color:TEXT, lineHeight:1.5}}>{"Les desks de NY ferment leurs livres. Le marché passe à l'Asie."}</div><div style={{fontSize:8, color:TEXT_DIM, lineHeight:1.5}}>{"\ud83e\udde0 Toi : tu sors, gagnant ou perdant. Toujours."}</div></div>
      </div>
      ...
      <div style={{padding:"12px 14px", background:"#0a1220", borderRadius:8, border:"1px solid #7dd3fc55", marginBottom:14}}>
        ...
        <div style={{fontSize:8.5, color:TEXT, lineHeight:1.7, marginBottom:8}}><b style={{color:"#fbbf24"}}>{"7h45 ou 9h — Tu vérifies le terrain : le pôle."}</b>{" Ton M15 montre-t-il un flux continu depuis 3h-4h (leur conviction) ? C'est la jambe des deux figures : tu y traceras ton Fib (retracement 9h30) ou tu guetteras la cassure de son flag (cassure 8h). Pas de pôle propre = ils hésitent = tu t'abstiens."}</div>
        <div style={{fontSize:8.5, color:TEXT, lineHeight:1.7, marginBottom:8}}><b style={{color:"#fbbf24"}}>{"7h45 ou 9h — Tu choisis le sentiment : ③."}</b>{" Un regard sur le Risk Meter : RISK-ON ou RISK-OFF ? Si tu joues le retracement 9h30, les news US de 8h30 sont passées — vérifie qu'elles n'ont pas retourné l'humeur. Un clic, c'est mémorisé."}</div>
        <div style={{fontSize:8.5, color:TEXT, lineHeight:1.7, marginBottom:8}}><b style={{color:"#fbbf24"}}>{"7h45-8h45 ou 9h-9h30 — Tu colles MarketMilk et tu scannes : ①."}</b>{" Le Currency Strength agrège ce que les desks ont FAIT depuis 3h : quelle devise reçoit le capital, laquelle le perd. ≥3 rangs dans le sens du pôle + ③ aligné + pas Least Volatile (❄️) = feu vert. Le scan vaut pour les deux figures."}</div>
        <div style={{fontSize:8.5, color:TEXT, lineHeight:1.7, marginBottom:8}}><b style={{color:"#4ade80"}}>{"8h ou 9h30 — Tu entres, selon ta figure."}</b>{" CASSURE 8h : à 8h le forex NY double la liquidité, le flag de Londres casse dans le sens du pôle — tu entres sur la clôture M15 confirmée, stop large pour survivre au balayage de 9h30. RETRACEMENT 9h30 : à 9h30 le NYSE retrace, tu entres au rebond Fib (38.2/50/61.8%) sur M5, stop sous le retracement. Target = hauteur du pôle. Tu n'as rien deviné : tu as lu leur trace et tu montes derrière eux."}</div>
        <div style={{fontSize:8.5, color:TEXT, lineHeight:1.7, padding:"8px 10px", background:"#0d1828", borderRadius:5}}>{"⚡ En une phrase : les desks construisent le pôle (3h-9h30), tu lis leur trace (① + ③ au scan), et tu montes derrière eux — soit sur la cassure fraîche de 8h (cassure 8h), soit dans le creux du retracement de 9h30 (retracement 9h30). Si un maillon manque — pas de pôle, pas de 3 rangs, sentiment contraire, paire gelée — la chaîne est cassée et tu passes. UNE seule entrée par jour. La discipline du non-trade EST le système."}</div>
      </div>
